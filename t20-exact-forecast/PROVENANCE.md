...
**The data.** The calibration constants are derived from the Cricsheet Indian Premier League ball-by-ball archive, maintained by Stephen Rushe: 1,243 matches and 295,557 deliveries, parsed by `dev/parse_archive.py`. Cricsheet publishes its data under the Open Data Commons Attribution License 1.0, and `league/calibration.json` carries that attribution. No Cricsheet row is redistributed; only aggregates ship, and the raw archive is not tracked in the repository. The constants were reproduced from the raw archive by the scripts under `dev/` to within 0.0007 of an earlier build. The task's worlds are freshly simulated leagues with invented players, teams and grounds; they are not renamed IPL fixtures.
