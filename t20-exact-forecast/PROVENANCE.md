...
**The data.** The calibration constants are derived from the Cricsheet Indian Premier League ball-by-ball archive, maintained by Stephen Rushe: 1,243 matches and 295,557 deliveries, parsed by `dev/parse_archive.py`.[^1] Cricsheet publishes its data under the Open Data Commons Attribution License 1.0, and `league/calibration.json` carries that attribution. No Cricsheet row is redistributed; only aggregates ship, and the raw archive is not tracked in the repository. The constants were reproduced from the raw archive by the scripts under `dev/` to within 0.0007 of an earlier build.[^2] The task's worlds are freshly simulated leagues with invented players, teams and grounds; they are not renamed IPL fixtures.
...
| The job records under `jobs/` | The two gates, the linter, and the sixteen graded model jobs of both generations, each with its reward, per-world details, agent log and submitted program; the two stopped jobs with their agent logs |
...
[^1]: the script's printed counts; NOTES.md, the parser section
[^2]: NOTES.md, the calibration file section
[^3]: RUN_REPORT.md, Sections 5 and 6, with job identities
