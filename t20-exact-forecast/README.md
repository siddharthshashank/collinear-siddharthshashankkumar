...
The pass bar is 1.10 times a reference forecaster built from ordinary statistics. The pair the brief's goal line names failed ten times out of ten, all for the same reason: they took players' short histories at face value and had no check that could see it; changing that one number in each program moved it most of the way to the reference. The next generation passed four of five completed runs by doing exactly what the failures skipped. So the bar sits between the two generations. Three further runs were cut short for account or operator reasons and are recorded as excluded, not as failures.
