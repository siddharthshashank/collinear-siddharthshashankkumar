...
grows quickly when a forecast is confident and wrong. Forecasts are clipped to [{clip}, {one_minus_clip}]. Your regret on a league is the
...
if your regret, summed over all eight leagues, is at most {relative_pct} percent above the reference's sum, and the same holds for the seven
